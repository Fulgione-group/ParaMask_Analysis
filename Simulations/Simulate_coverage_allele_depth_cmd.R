...
matpath<- args[1]
outpath <- paste(paste(strsplit(matpath,split = "\\.")[[1]][1:(length(strsplit(matpath,split = "\\.")[[1]])-1)] , collapse = "."), ".vcf", sep="")
...
convert_snp<- function(snp, rho){
  ...
  if(startsWith(as.character(snp[length(snp)]),"SV")){
    SV <- TRUE
    cov <- rnegbin(n = (length(snp)-3), mu = 20, theta =8)
  }else{
    SV<-FALSE
    cov <- rnegbin(n = (length(snp)-3), mu = 10, theta = 8)
  }
  cov[cov<0] <- 0
