...
fn_ES03 <- read.table("~/Downloads/ES03_FN_SNPs_hetGen_of_last_seed_SNPs.txt", header = T)
head(fn_ES03)
...
fn_ES04 <- read.table("~/Downloads/ES04_FN_SNPs_hetGen_of_last_seed_SNPs.txt", header = T)
head(fn_ES04)
...
####compare single SVs
SVs <-read.table("~/Data/Paramask/SVs/ES03_014_only_overlapping_dup_with_header.txt", header = T)
head(SVs)
bed<-read.table("ParaMask_additional_data/ParaMask_results/SpanishArabisAlpina/ES0304_run_finalEMresults.chr1.finalClass.bed", header = T)

###

# Find overlaps
# Get the full structure from bed so we can ensure consistency
bed_template <- bed[0, ]
bed_template$SV_ID <- integer(0)
bed_template$relStart <- numeric(0)
bed_template$relEnd <- numeric(0)
SVs<- SVs[SVs$Chromosome=="chr1",]
# Loop with consistent output structure
overlaps_list <- lapply(1:nrow(SVs), function(i) {
  sv <- SVs[i, ]
  overlapping <- bed[bed$Start <= sv$STOP & bed$End >= sv$START, ]
  if (nrow(overlapping) == 0) {
    # Return a row with NAs but correct columns
    empty_row <- bed_template[1, ]
    empty_row[1, ] <- NA
    empty_row$SV_ID <- i
    empty_row$relStart <- NA
    empty_row$relEnd <- NA
    return(empty_row)
  } else {
    overlapping$SV_ID <- i
    overlapping$relStart <- overlapping$Start - sv$START
    overlapping$relEnd <- overlapping$End - sv$START
    return(overlapping)
  }
})
...
# Prepare SVs with relative positions (all start at 0)
SVs$SV_ID <- 1:nrow(SVs)
SVs$relStart <- 0
SVs$relEnd <- SVs$STOP - SVs$START
SVs$Type <- "SV"

overlaps$Type <- "Call"
# Add dummy column to SVs to match overlaps
SVs$type.0.single.copy.1.multi.copy <- NA

# Make sure the extra column exists in SVs
SVs$type.0.single.copy.1.multi.copy <- NA

# Now bind both with matching columns
plot_data <- rbind(
  SVs[, c("SV_ID", "relStart", "relEnd", "Type", "type.0.single.copy.1.multi.copy")],
  overlaps[, c("SV_ID", "relStart", "relEnd", "Type", "type.0.single.copy.1.multi.copy")]
)

library(ggplot2)

ggplot(plot_data, aes(x = relStart, xend = relEnd, y = Type, yend = Type)) +
  geom_segment(data = subset(plot_data, Type == "SV"),
               color = "blue", size = 2) +
  geom_segment(data = subset(plot_data, Type == "Call"),
               aes(color = factor(type.0.single.copy.1.multi.copy)),
               size = 2, na.rm = TRUE) +
  scale_color_manual(values = c("0" = "grey60", "1" = "red"), na.translate = FALSE) +
  facet_wrap(~SV_ID, scales = "free_x") +
  labs(x = "Position relative to SV start", y = NULL, color = "Copy Type") +
  theme_minimal() +
  theme(
...
SVs <-read.table("~/Data/Paramask/SVs/ES03_014_only_overlapping_dup_with_header.txt", header = T)
head(SVs)
bed<-read.table("ParaMask_additional_data/ParaMask_results/SpanishArabisAlpina/ES0304_run_finalEMresults.chr2.finalClass.bed", header = T)
...
SVs <-read.table("~/Data/Paramask/SVs/ES03_014_only_overlapping_dup_with_header.txt", header = T)
head(SVs)
bed<-read.table("ParaMask_additional_data/ParaMask_results/SpanishArabisAlpina/ES0304_run_finalEMresults.chr3.finalClass.bed", header = T)
